<template>
<div class="v-popup-torgi">
    <div class="center">
    <div class="container">
        <div class="title">Предложить цену</div>
        ...
        <span>Стоимость:</span>
        <input v-model="totalCost" type="number" id="cost" placeholder="Цена" />
        </div>
        ...
        <input type="text" id="local" placeholder="Геолокация" />
        </div>

        <div class="container_center">
        <button @click="toOrder" class="button_yellow_border">Ок</button>
        </div>
    </div>
    </div>
  ...
  async toOrder() {
    ...
    const startDate = new Date(
      ...
      const response = await axios.post(
          "http://localhost:8080/regOrderBidding",
          {
  ...
  async printAds() {
    ...
    try {
      ...
      if (response.data.status != "success") console.log(response);
        ...
        if(response.data.data.Hourly_rate != 0) {
          this.hourlyRate = response.data.data.Hourly_rate;
        } else {
          this.dailyRate = response.data.data.Daily_rate;
        }
      }
