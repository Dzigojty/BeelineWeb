<template>
    <div class="v-popup-auth">
      ...
      <div v-if="showAuth == 'password'" class="center">
        ...
        <form @submit.prevent="validateEmailRePassword">
      ...
      <div v-if="showAuth == 'vostPassword'" class="center">
        <div class="title">Восстановление пароля</div>
        <form @submit.prevent="validatePassword">
          <div class="small-text-black marg-bottom">Придумайте пароль</div>
          <input v-model="password" type="password" />
  ...
  export default {
    ...
    methods: {
        ...
        const response = await axios
          ...
          .then(function (response) {
            ...
            if (response.data.Status == "fatal") {
              ...
              localStorage.setItem('User_role', response.data.data.Information.User_role);
              localStorage.setItem('Rating', response.data.data.Information.Rating);
      ...
      validateCode() {
        this.errors = {};
        console.log(this.passwordRecoveryForm.emailOrPhone);
        if (!this.passwordRecoveryForm.emailOrPhone) {
          this.errors.recoveryCode = "Введите адрес электронной почты.";
        } else if (!this.isValidEmail(this.passwordRecoveryForm.emailOrPhone)) {
          this.errors.recoveryCode = "Введите корректный код.";
        }
      ...
      validateEmailRePassword() {
        this.errors = {};
        console.log(this.passwordRecoveryForm.emailOrPhone);
        if (!this.passwordRecoveryForm.emailOrPhone) {
          this.errors.emailOrPhone = "Введите адрес электронной почты.";
        } else if (!this.isValidEmail(this.passwordRecoveryForm.emailOrPhone)) {
          this.errors.emailOrPhone = "Введите корректный email.";
        }
